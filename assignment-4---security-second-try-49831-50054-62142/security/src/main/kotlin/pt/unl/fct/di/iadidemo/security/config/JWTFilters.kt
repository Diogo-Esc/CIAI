...
import pt.unl.fct.di.iadidemo.security.domain.UserTokenDAO
import java.util.*
...
class UserPasswordAuthenticationFilterToJWT (
    ...
    override fun successfulAuthentication(request: HttpServletRequest,
        ...
        val authHeader = (response as HttpServletResponse).getHeader("Authorization")
        ...
        val uName = claims["username"] as String
        val user = users.findUser(uName).get()
        user.userToken = listOf(UserTokenDAO(token, true, user)) //token saved
        users.addUser(user) //user updated
...
class JWTAuthenticationFilter(private val users: UserService): GenericFilterBean() {
    ...
    override fun doFilter(request: ServletRequest?,
        ...
        if( authHeader != null && authHeader.startsWith("Bearer ") ) {
            ...
            val user = users.findUser(claims["username"] as String).get()
            ...
            if ( exp < System.currentTimeMillis()/1000) // in seconds
                ...
                val authentication = UserAuthToken(claims["username"] as String, listOf(SimpleGrantedAuthority("USER")))
...
class UserPasswordSignUpFilterToJWT (
    ...
    override fun attemptAuthentication(request: HttpServletRequest?,
        ...
        return users
                ...
                .orElse( null )
                .let {
                    val auth = UserAuthToken(user.username,listOf(SimpleGrantedAuthority("USER")))
                    SecurityContextHolder.getContext().authentication = auth
                    auth
...
class UserPasswordSignOutFilterToJWT (
    ...
    override fun attemptAuthentication(request: HttpServletRequest?,
        ...
        //getting user from request body
        val user = ObjectMapper().readValue(request!!.inputStream, UserDAO::class.java)
        /*val u = users.findUser(user.username).get()
        u.userToken[0].token = "revoked"
        u.userToken[0].valid = false
        users.addUser(u)*/
        ...
        return if (auth.isAuthenticated) {
            // Proceed with an authenticated user
            auth.isAuthenticated = false //revoke user authentication
            SecurityContextHolder.getContext().authentication = auth
            auth
        } else
            null
