...
class BookController(val books: BookService, val authors: AuthorService) : BooksAPI {
    ...
    override fun getAll(): List<BookListDTO> =
        ...
        override fun addOne(elem: BookDTO): BookListDTO{
            ...
            val b = BookDAO(0, elem.title, authors.toMutableList(), elem.images.map { ImageDAO(0, it ) })

            books.addOne(b);

            return BookListDTO(b.id, b.title, authors.map { AuthorsBookDTO(it.name) }, elem.images.map { ImageDTO(it) });
